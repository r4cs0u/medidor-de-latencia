(function () {
  ...
  function globalXRange(channels, refT0Ms) {
    ...
    if (!isFinite(xMin)) return { xMin: 0, xMax: 30 };
  ...
  async function openPanel() {
    ...
    function getRefT0() {
      ...
      const arr = ref.rollingBuffer.toArray();
    ...
    function updateOverlay(channels, refT0, xMin, xMax, ticks) {
    ...
    function buildParallel(channels) {
      ...
      channels.forEach((ch, idx) => {
        ...
        liveRefs[ch.id] = { ci, anchorXSecsRef };
      });
    }
    ...
    function buildOverlay(channels) {
      ...
      const ci = new Chart(cvs, {
        type: 'line',
        data: { labels: channels[0] ? datasets[0].data.map(p => p.x) : [], datasets },
        options: opts,
